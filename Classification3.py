...
def main(subjectname,count,collectioncount):
    start = time.time()
    ...
    #File read
    start = time.time()
    path = f"victimpickle/{collectioncount}/"
    os.makedirs(path, exist_ok=True)

    all_csv_data = asp.openJoblibDump(f'allsignpickle/{subjectname}_data.joblib')
    all_csv_name = asp.openJoblibDump(f'allsignpickle/{subjectname}_sign.joblib')
    """
    all_csv_data = asp.openPickel(f'pickle/Subject{count}_data.pickle')
    all_csv_name = asp.openPickel(f'pickle/Subject{count}_sign.pickle')
    """
...
if __name__ == '__main__':
    ...
    #Multi subject mode
        ...
        for (i,name) in enumerate(subjectname):
            if os.path.exists(f'victimpickle/{collectioncount}/Subject{i}_data.pickle') == False:
                main(name,i,collectioncount)
            else:
                print(f'victimpickle/{collectioncount}/Subject{i}_data.pickle is already exist')